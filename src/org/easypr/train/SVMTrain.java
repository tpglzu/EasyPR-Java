...
import org.bytedeco.javacpp.opencv_ml.ParamGrid;
import org.bytedeco.javacpp.opencv_ml.SVM;
import org.bytedeco.javacpp.opencv_ml.StatModel;
import org.bytedeco.javacpp.opencv_ml.TrainData;
import org.easypr.scanner.core.CashFeatures;
import org.easypr.scanner.detect.SVMCallback;
...
public class SVMTrain {
	...
	private SVMCallback callback = new CashFeatures();
...
//			Mat features = this.callback.getHisteqFeatures(img);
	...
	private void getPlateTest(Vector<Mat> testingImages,
	...
	public void getHasPlateTest(Vector<Mat> testingImages,
			Vector<Integer> testingLabels) {
		getPlateTest(testingImages, testingLabels, hasPlate);
	}

	public void getNoPlateTest(Vector<Mat> testingImages,
			Vector<Integer> testingLabels) {
	...
	public void getAccuracy(Mat testingclasses_preditc, Mat testingclasses_real) {
		...
		for (int i = 0; i < nRows; i++) {
			...
			final float predict = Convert
					.toFloat(testingclasses_preditc.ptr(i));
	...
	public int svmTrain(boolean dividePrepared, boolean trainPrepared) {
		...
		if (!trainPrepared) {
			...
			Mat aa = new Mat(labels); 
			aa.copyTo(classes);
			aa.close();
		}

		// Test SVM
		Vector<Mat> testingImages = new Vector<Mat>();
		...
		if (true && !trainPrepared && !classes.empty() && !trainingData.empty()) {
			TrainData trainData = TrainData.create(trainingData, 0, classes);
			trainData.setTrainTestSplitRatio(0.1);
			...
			try {
				...
				svm.setNu(0.1);
				svm.setP(0.1);
				svm.setTermCriteria(new TermCriteria(CV_TERMCRIT_ITER, 20000, 0.0001));
				
				svm.train(trainData);
				
				// CvSVM svm(trainingData, classes, Mat(), Mat(), SVM_params);
//				svm.trainAuto(trainData, 10, new ParamGrid(SVM.C),
//						new ParamGrid(SVM.GAMMA), new ParamGrid(SVM.P),
//						new ParamGrid(SVM.NU), new ParamGrid(SVM.COEF),
//						new ParamGrid(SVM.DEGREE), true);
			} catch (Exception err) {
...
//			CvFileStorage fsTo = CvFileStorage.open("res/train/svm.xml",
//					CvMemStorage.create(), CV_STORAGE_WRITE);
//			svm.write(new FileStorage(fsTo));
			svm.save("res/train/svm.xml");
		} else {
			try {
				String path = "res/train/svm.xml";
				svm = SVM.loadSVM(path, null);
			} catch (Exception err) {
