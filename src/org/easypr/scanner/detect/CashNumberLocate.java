...
public class CashNumberLocate {
     ...
     * 
     */
    public void setLifemode(int islifemode) {
    	switch (islifemode) {
		case 1:
...
//        GaussianBlur(src, src_blur, new Size(gaussianBlurSize, gaussianBlurSize), 0, 0, BORDER_DEFAULT);
        GaussianBlur(src, src_blur, new Size(35, 35), 0, 0, BORDER_DEFAULT);

        if (debug) {
            saveTmp("debug_GaussianBlur.jpg", src_blur);
        }

        // Convert it to gray 将图像进行灰度化
        cvtColor(src_blur, src_gray, CV_RGB2GRAY);
        if (debug) {
            saveTmp("debug_gray.jpg", src_gray);
        }
        ...
        if (debug) {
            saveTmp("debug_threshold.jpg", img_threshold);
        }
        ...
        if (debug) {
            saveTmp("debug_morphology.jpg", img_threshold);
        }
        ...
        if (debug) {
            ...
            saveTmp("debug_Contours.jpg", result);
        }
        ...
        if (debug) {
            saveTmp("debug_result.jpg", result);
        }
    ...
    private boolean verifySizes(RotatedRect mr) {
        ...
        int min = 100 * 15 * verifyMin; // minimum area
        int max = 100 * 15 * verifyMax; // maximum area
        ...
        return area >= 2000 && r >= rmin && r <= rmax;
    ...
    private Mat showResultMat(Mat src, Size rect_size, Point2f center, int index) {
        ...
        if (debug) {
            saveTmp("debug_crop_" + index + ".jpg", img_crop);
        }
        ...
        if (debug) {
            saveTmp("debug_resize_" + index + ".jpg", resultResized);
        }
